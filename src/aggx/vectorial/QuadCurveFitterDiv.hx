	...
	private function recursiveBezier(x1:Float, y1:Float, x2:Float, y2:Float, x3:Float, y3:Float, level:Int):Void {
		...
		if (d > CURVE_COLLINEARITY_EPSILON) {
			...
			if (da == 0) {
				...
				if (d <= 0)
					d = Calc.squaredDistance(x2, y2, x1, y1);
				else if (d >= 1)
					d = Calc.squaredDistance(x2, y2, x3, y3);
				else
					d = Calc.squaredDistance(x2, y2, x1 + d * dx, y1 + d * dy);
			}
