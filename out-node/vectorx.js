...
ExampleVectorx.main = function() {
	...
	var fontContext = new vectorx_font_FontContext();
	var font = fontCache.createFontWithNameAndSize("Arial",35.0);
	var attachmentColorStorage = ExampleVectorx.renderAttachment();
	var attachment = new vectorx_font_FontAttachment(function() {
		return attachmentColorStorage;
	},0,0,70,32);
	var red = new types_Color4F();
	red.r = 1;
	red.g = 0;
	red.b = 0;
	red.a = 1;
	...
	if(__map_reserved["a1"] != null) {
		_g.setReserved("a1",attachment);
	} else {
		_g.h["a1"] = attachment;
	}
	var attachments = _g;
	var attributedString = new vectorx_font_AttributedString("ABC",{ range : new vectorx_font_AttributedRange(), font : font, backgroundColor : red, attachmentId : "a1"});
	var colorStorage = new vectorx_ColorStorage(500,500);
	...
	var layoutConfig = { scale : 1, horizontalAlignment : types_HorizontalAlignment.Center, verticalAlignment : types_VerticalAlignment.Top, layoutBehaviour : vectorx_font_LayoutBehaviour.AlwaysFit};
	console.log("src/ExampleVectorx.hx:96:",layoutConfig);
	console.log("src/ExampleVectorx.hx:104:",fontContext.calculateTextLayout(attributedString,colorStorage.selectedRect,layoutConfig,function(id,ratio) {
		return __map_reserved[id] != null ? attachments.getReserved(id) : attachments.h[id];
	}));
};
...
var aggx_rfpx_data_CmapFormat4 = function(data) {
	...
	i = 0;
	while(_$UInt_UInt_$Impl_$.gt(segCount,i)) {
		var this11 = this._startCount;
		var x07 = data.readUInt16();
		this11[i] = (x07 & 255) << 8 | x07 >> 8 & 255;
		data.set_offset(data.get_offset() + 2);
		++i;
	}
	...
	i = 0;
	while(_$UInt_UInt_$Impl_$.gt(segCount,i)) {
		var this12 = this._idDelta;
		var x08 = data.readUInt16();
		this12[i] = ((x08 & 255) << 8 | x08 >> 8 & 255) << 16 >> 16;
		data.set_offset(data.get_offset() + 2);
		++i;
	}
	...
	i = 0;
	while(_$UInt_UInt_$Impl_$.gt(segCount,i)) {
		var this13 = this._idRangeOffset;
		var x09 = data.readUInt16();
		this13[i] = ((x09 & 255) << 8 | x09 >> 8 & 255) << 16 >> 16;
		data.set_offset(data.get_offset() + 2);
		++i;
	}
...
var aggx_rfpx_data_LocaTable = function(record,data,numGlyphs,offsetsType) {
	...
	if(offsetsType == 0) {
		...
		while(_$UInt_UInt_$Impl_$.gt(numGlyphs,i)) {
			var this11 = this._offsets;
			var x01 = data.readInt32();
			this11[i] = (x01 >>> 0 & 255) << 24 | (x01 >>> 8 & 255) << 16 | (x01 >>> 16 & 255) << 8 | x01 >>> 24 & 255;
			data.set_offset(data.get_offset() + 4);
			++i;
		}
	}
		...
		while(_g < _g1) {
			...
			while(_g2 < _g11.length) {
				...
				if(span1.strokeWidth != null) {
					...
					var strokeWidth = Math.abs(span1.strokeWidth);
					fontEngine.renderStringStroke(spanString,span1.getFontSize() * pixelRatio,x,spanY,scanlineRenderer,strokeWidth,kern);
				}
				...
				if(span1.attachment != null) {
					...
					var srcOffset = srcData.get_offset();
					var dstOffset = dstData.get_offset();
					var spanY1 = y + (line.maxSpanHeight - attachment.bounds.height + attachment.heightBelowBaseline()) + baseLineOffset;
					vectorx_font_FontContext.blendFromColorStorage(Math.ceil(x),Math.ceil(spanY1),outStorage,attachment.get_image(),attachment.bounds);
